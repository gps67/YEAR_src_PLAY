...
class CDDB_Album:
 ...
 def set_attr_list( self, key, val ):
 	# EXTD #
 	if 0:
 		print(( "# ALBUM set_attr_list( %-10s, %s )" % ( key, val )))
 	val = val.strip()
 	if not val:
 		return
 	try:
 ...
 def regen_cddb_attr_list( self, key, once=False ):
 	...
 	for val in lst:
 		val = val.strip()
 		self.out.append_unicode_ln( '%s=%s' % (key, val ))
 ...
 def txt_cddb_attr( self, out, key ):
 	...
 	if val == None: return
 	val = val.strip()
 	out.append_unicode_ln( '%-10s %s' % (key, val ))

 def regen_cddb_DTITLE( self ):
 	DTITLE = "DTITLE"
 	if self.__dict__[DTITLE]:
 		self.__dict__[DTITLE] = self.DTITLE_ARTIST + " / " + DTITLE_ALBUM
 ...
 def gen_cddb_text_out( self ):
 	...
 	out = self.out
 	out.append_unicode_ln( '# xmcd CD database file generated by %s' % self.cddb_submitted_via)
 	out.append_unicode_ln( '# ')
 	out.append_unicode_ln( '# Track frame offsets:')
 	for trk in self.TRACK:
 		out.append_unicode_ln( "#        %s" % trk.offset)
 	out.append_unicode_ln( '# ')
 	out.append_unicode_ln( '# Disc length: %s seconds' % self.seconds)
 	out.append_unicode_ln( '# ')
 	rev = self.cddb_revision
 	if rev == "0" : rev = ""
 	out.append_unicode_ln( '# Revision: %s' % rev )
 	out.append_unicode_ln( '# Submitted via: %s' % self.cddb_submitted_via)
 	out.append_unicode_ln( '# ')
 	self.regen_cddb_attr( "DISCID" )
 	self.regen_cddb_attr( "DTITLE" )
 	self.regen_cddb_attr( "DYEAR" )
 	self.regen_cddb_attr( "DGENRE" )
 	self.regen_cddb_attr( "DID3" )
 	...
 	self.regen_cddb_attr_list( "EXTD" )
 	self.regen_cddb_attr( "PLAYORDER" ) # LURN should be submitted empty
...
# GPS # CALL #
 ...
 def txt_cddb_summary( self ):
 	out = unicode_buffer()
 	out.append_unicode_ln( '# ')
 	self.txt_cddb_attr( out, "DTITLE_ARTIST" )
 	self.txt_cddb_attr( out, "DTITLE_ALBUM" )
 	out.append_unicode_ln( '# ')
 	self.txt_cddb_attr( out, "DISCID" )
 	self.txt_cddb_attr( out, "DTITLE" )
 	self.txt_cddb_attr( out, "DYEAR" )
 	self.txt_cddb_attr( out, "DGENRE" )
 	# should be different format
 	# UNUSED self.regen_cddb_attr( "seconds" )
 	idx = -1
 	for trk in self.TRACK:
...
class CDDB_Parser:
 ...
 def conv_line( shelf, line, e1, e2 ,e3 ):
 	return line
	# this converts "str" to "bytes" # which is crap
	# TODO restart LOAD STRINGS from file 
	# TODO heuristic - looks like double encode 8859 latin1
	# TODO write out as latin1 or as utf8 ??
 	try:
 		if 0:
 			if dbg_bytes_uses_bit8( line_in ):
 				print(( "# encoding expected:", e1))
 				dbg_bytes( line_in )
 			line_unic = str( line_in, e1 )
 			if 1: dbg_uni( line_unic )
 		# else:
 		# line_unic = str( line_in, e1 )
 		line_unic = line_in.encode( e1 )
 	except:
 		...
 		try:
 			...
 			# WAS # line_unic = str( line_in, e2 )
 			line_unic = line_in.encode( e2 )
 		except:
 			# I dont see how this could happen
 			# WAS # line_unic = str( line_in, e3 )
 			line_unic = line_in.encode( e3 )
 	line_unic = line_unic.encode( "UTF8" )
 ...
 def conv_lines( shelf, lines, e1, e2 ,e3 ):
 	...
 	for line_in in lines:
 		line_unic = shelf.conv_line( line_in, e1, e2, e3 )
 ...
 def expect_blank_line( self ):
 	if not self.got_line():
 		...
 		raise Exception(  "blank_line_expected" )
 	if self.line == '':
 		self.line_used()
 	else:
 ...
 def RE_error_parse_fail_uses_line(self):
 	...
 	for re in self.fail_list:
 		print(( "TRIED: %s" % re))
 	self.fail_list = []
 	raise Exception(  "RE_error_parse_fail_uses_line()" )
 			...
 			if idx == "":
 				...
 				if key == "DTITLE":
 				 ...
 				 if self.Album.DTITLE_ARTIST:
 				 	...
 				 	val = val.strip();
 				 	print(( "val:", val))
 				 	DTITLE_ALBUM = self.Album.DTITLE_ALBUM + " " + val
 				 	self.Album.DTITLE_ALBUM = self.Album.DTITLE_ALBUM + " " + val
