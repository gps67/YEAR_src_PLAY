#ifndef spout_styles_H
#define spout_styles_H
...
/*!
	...
	Each Style_Atom has a virtual spout_gen() function, which calls a SPOUT
	function which generates a command in the SPOUT file (In a
...
*/
class Style_Atom : public spout_obj_ref
{ public:

	//! Every style can reach the single main SPOUT channel
	SPOUT_t * spout;
...
//		Change_Text_stretch_x,   // unused
	...
	Style_Atom( SPOUT_t * _spout, ChangeFlag _change_flag );
	virtual ~Style_Atom();
	virtual void pre_setup();
	virtual void spout_gen();
	virtual void invalidate_delete();
};

/*!
	There is only one single ACTIVE font@point, but SPOUT_GEN keeps
...
*/
class Style_Tray : public spout_obj_ref
{
 public:
	/*!
	...
	*/
	...
	Style_Tray( spout_PAGE_base * _spout);
	~Style_Tray();
...
class Style_Settings : public Style_Tray
{
 public:
	...
	*/
	...
	Style_Settings( spout_PAGE_base * _spout );
