...
)
{
	...
	INFO( "(%s) child process %d has exited with status %d",
...
bool Spawn::argv_add( str0 s )
{
	// INFO( s ); // loop detect
	return argv_add( (const char *) s );
}
...
bool Spawn::argv_fmt( const char * fmt, ... )
{
	bool ok = false;
        ...
        va_start( ap, fmt );
	buffer2 buf;
	ok = buf.vprint( fmt, ap );
        va_end( ap );
	argv_add( (str0) buf );
	return ok;
}

bool Spawn::argv_print_as_line( buffer2 & line )
{
	const char * fmt = "%s";
	for( int i = 0; i< arg_c; i++ ) {
		line.print(  fmt, (STR0) arg[i] );
		fmt = " %s";
	}
	return true;
}
...
// gdb_invoke();
	...
	buffer2 line;
	argv_print_as_line( line );
	STEP("(%s) RUN_ASYNC('%s') ... ", dgb_name.get_str(), (STR0) (str0) line );
 ...
 if(0) {
	...
	*/
	...
	if( t ) {
		...
		//
		INFO("(%s) g_spawn returned FAIL", dgb_name.get_str() ); // or use argv[0]
		/*
			THERE IS NO PID AND NO CHILD WATCH (see set_watch())
			INFO("PID == %d", child_pid );
			INFO("child_watch_event_source == %d", child_watch_event_source );
		*/
		only_print_gerror();
		/*
...
void Spawn::cb_child_exit()
{
	...
	INFO("(%s) CHILD PROCESS HAS EXITED WITH STATUS %d",
		dgb_name.get_str(),
		child_exit_status
	);
}
...
{
	INFO("(%s) CHILD PROCESS IS BEING KILLED - PRESUME TO EXIT", dgb_name.get_str());
...
bool Spawn::kill_pid( int unusedstatus )
{
	...
	if( !have_child_pid() ) {
		if(0) INFO("OK %s child_pid is NULL", dgb_name.get_str() );
		return true;
	}

#ifdef WIN32
	STEP("(%s) killing with status (not signal) = %d", dgb_name.get_str(), unusedstatus );
...
*/
	...
	STEP( "(%s) killing %d with signal (not status) = %d",
		dgb_name.get_str(),
		child_pid,
		SIG
	);
	...
	if (kill( child_pid, SIG)) {
		FAIL("(%s) kill() failed", dgb_name.get_str());
	}
#endif
