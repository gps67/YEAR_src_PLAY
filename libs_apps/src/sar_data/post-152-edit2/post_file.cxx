...
inline bool post_file::recover_file()
{
    ...
    size_t page_map_size = ((size / page_size) + 31) >> 5;
    int* recovered_page_map = new int[page_map_size];
    memset(recovered_page_map, 0, sizeof(int)*page_map_size);
    ...
    while (ReadFile(log, log_buffer, trans_size, &read_bytes, NULL)) { 
        ...
        assert(offs < size);
        if ((recovered_page_map[page_no >> 5] & (1 << (page_no & 0x1F))) == 0) { 
            recovered_page_map[page_no >> 5] |= 1 << (page_no & 0x1F);
...
bool post_file::create_shadow_page(int modify, void* addr)
{
    ...
    *(ptrdiff_t*)log_buffer = offs;
    ...
    if (platform == VER_PLATFORM_WIN32_NT 
        && !VirtualProtect(page, page_size, modify ? PAGE_READWRITE : PAGE_READONLY, &old_prot)) 
    { 
        TRACE_MSG(("post_file::create_shadow_page: VirtualProtect %p failed: %ld\n",
                   page, GetLastError()));
        return false;
...
bool post_file::handle_page_modification(void* addr) 
{
    ...
    for (post_file* fp = chain; fp != NULL; fp = fp->next) { 
        if (size_t((char*)addr - fp->base) < fp->mapped_size) { 
            return fp->prot != read_only
                && fp->create_shadow_page(true, addr);
        }
    }  
    return false;
} 
...
static void fatal_error_handler(int signo) { 
	...
	return;
    char  buf[64];
    const char* exe_name = getenv("_"); // defined by BASH
    if (exe_name == NULL) { 
        exe_name = "a.out";
    }
    fprintf(stderr, "\
